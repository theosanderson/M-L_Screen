...
variance <- narrow %>% group_by(Pool, Condition, Mouse, gene) %>% 
  summarise(sd = sd(log_proportion), val1 = log_proportion[1], 
    val2 = log_proportion[2], meanlog_proportion = mean(log_proportion))
```
...
new<-new%>% group_by(gene,sample) %>%  dplyr::summarise(log_proportion=mean(log_proportion))
bla<-spread(new,key=sample,value=log_proportion,fill=-50) %>% ungroup() %>% select(-gene)
...
#variance<-filter(variance, !(Pool=="W" & Mouse==3))
#variance<-filter(variance, !(Pool=="Y" & Mouse==3))
```
...
```{r}
variance<-variance %>% group_by(Pool,Condition,Mouse) %>% arrange(-meanlog_proportion) %>% mutate(medsd=rollmean(x=sd,k=11, fill="extend"))
...
```{r}
ggplot(variance,aes(x=meanlog_proportion,y=monotonicmedsd,color=Condition))+
...
comparison<- comparison %>% mutate(difference=meanlog_proportion.y-meanlog_proportion.x)
...
unnormalised=comparison  %>% filter(Condition=="Rec")
basic<-mergeMultiples(unnormalised)
write_csv(basic,"unnormalised.csv")
```
...
normalisedQuarter=comparison  %>% filter(Condition=="Rec")%>% normaliseWithMostAbundantQuarter() %>% mergeMultiples() %>% mutate(version="withabundant") 

normalisedQuarterBlood=comparison  %>% filter(Condition=="Rec")%>% normaliseWithMostAbundantQuarter() %>% bloodStageNormalise() %>% mergeMultiples() %>% mutate(version="withabundant") 
...
unnormalisedatall=comparison %>% filter(Condition=="Rec") %>% mergeMultiples()
justnormalisedQuarter=comparison %>% filter(Condition=="Rec") %>% normaliseWithMostAbundantQuarter()%>% mergeMultiples()
roc1<-rocAnalysis(comparison %>% filter(Condition=="Rec") %>%bloodStageNormalise() %>% mergeMultiples())
roc2<-rocAnalysis(comparison %>% filter(Condition=="Rec") %>% mergeMultiples())
```
```{r}
roc<-read_csv("roc.csv")
roc$measure=roc$JustInjection
roc<-roc %>% filter(!is.na(measure))
library(ROCR)

rocAnalysis<-function(df,name=NULL){
  df<-filter(df,Pool!="Pilot")
  merge<-inner_join(df,roc,by=c("gene"="Old"))
  merge$diffB=merge$difference+merge$differencesd
  pred=prediction(-merge$difference,merge$measure)
  perf<-performance(pred,"prec",x.measure="rec")
  plot(perf)
  
  df=tibble(recall=unlist(perf@x.values),  precision=unlist(perf@y.values),cutoff=unlist(perf@alpha.values))
  df$name=name
  return(df)
}

a1<-rocAnalysis(normalisedQuarter,"normalisedQuarter")
a2<-rocAnalysis(justnormalisedQuarter,"justnormalisedQuarter")
a3<-rocAnalysis(normalisedQuarter,"normalisedQuarter")
a4<-rocAnalysis(normalisedControls,"normalisedControls")
a5<-rocAnalysis(unnormalisedatall,"unnormalisedatall")
a<-bind_rows(a1,a2,a3,a4,a5)
ggplot(filter(a),aes(y=precision,x=recall,color=name,group=name))+geom_line()

final<-normalisedQuarter %>% mutate(hit=difference< -2)
filter(final,hit==TRUE) %>% group_by(Pool) %>% summarise(n=n())
```
...
rocAnalysis<-function(df,name=NULL){
  ...
  merge<-inner_join(df,roc,by=c("gene"="Old"))
  merge$diffB=merge$difference+merge$differencesd
  pred=prediction(-merge$diffmax,merge$measure)
  perf<-performance(pred,"prec",x.measure="rec")
  plot(perf)
