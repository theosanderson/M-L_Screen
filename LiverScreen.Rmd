...
normaliseWithLeastReducedQuarter <- function(unnormaliseddf) {
  ...
  norms
  comparison_norm <- inner_join(comparison_norm, norms, suffix = c("", 
...
normalisedQuarter=comparison  %>% filter(Condition=="Rec")%>% normaliseWithLeastReducedQuarter() %>% mergeMultiples() %>% mutate(version="withabundant") 
