...
justnormalisedQuarter=comparison %>% filter(Condition=="Rec") %>% mergeMultiples()
...
```
```{r}
roc<-read_csv("roc.csv",col_names=F)
library(ROCR)

rocAnalysis<-function(df){
  merge<-inner_join(df,roc,by=c("gene"="X1"))
  merge$diffB=merge$difference+merge$differencesd
  pred=prediction(-merge$difference,merge$X3)
  perf<-performance(pred,"prec",x.measure="rec")
  plot(perf)
  
}

rocAnalysis(unnormalisedatall)
```{r}
